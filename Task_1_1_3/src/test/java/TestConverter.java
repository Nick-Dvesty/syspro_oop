...
public class TestConverter {
    ...
    public void testSubstitution(){
        ...
        Assertions.assertEquals(12, vr.substitution("x = 2"));
        vr = Expression.convertor("40-3*x+5*y");
        Assertions.assertEquals(-65, vr.substitution("x = 40; y = 3"));
        vr = Expression.convertor("(40+3*x)*(5-y)");
        Assertions.assertEquals(0, vr.substitution("x = 2; y = 5"));
        vr = Expression.convertor("(40+3*x)+(5-y)");
        Assertions.assertEquals(47, vr.substitution("x = 2; y = 4"));
        vr = Expression.convertor("(40+3*x)+(5+y)/2");
        Assertions.assertEquals(50.5d, vr.substitution("x = 2; y = 4"));
        Assertions.assertThrows(ArithmeticException.class, () -> {
            var vr2 = Expression.convertor("(40+3*x)+(5+y)/0");;
                vr2.substitution("x = 2; y = 4");
        });
        Assertions.assertThrows(ArithmeticException.class, () -> {
    ...
    @Test void testDifferentiation(){
        ...
        Assertions.assertEquals(vr.dif("y").print(), "(0+1)" );
        vr = new Add(new Variable("x"), new Variable("y"));
        Assertions.assertEquals(vr.dif("x").print(), "(1+0)" );
        Assertions.assertEquals(vr.dif("z").print(), "(0+0)" );
        vr = new Sub(new Variable("xi"), new Number("1"));
        Assertions.assertEquals(vr.dif("xi").print(), "(1-0)" );
        vr = new Mul(new Variable("xi"), new Number("32"));
        Assertions.assertEquals(vr.dif("xi").print(), "((1*32)+(xi*0))");
        vr = new Div(new Variable("xi"), new Number("32"));
        Assertions.assertEquals(vr.dif("xi").print(), "(((1*32)-(xi*0))/(32*32))");
    }

    @Test void testSimple(){
        Expression vr = Expression.convertor("4+y");
        Assertions.assertEquals(vr.simple().print(), "(4+y)" );
        Assertions.assertEquals(vr.dif("y").simple().print(), "1" );
        vr = Expression.convertor("4-y");
        Assertions.assertEquals(vr.simple().print(), "(4-y)" );
        Assertions.assertEquals(vr.simple().dif("y").print(), "(0-1)" );
        vr = Expression.convertor("(23*x)-(23*x)");
        Assertions.assertEquals(vr.simple().print(), "0" );
        vr = Expression.convertor( "x+y");
        Assertions.assertEquals(vr.dif("x").simple().print(), "1" );
        Assertions.assertEquals(vr.dif("z").simple().print(), "0" );
        vr = Expression.convertor("xi+1");
        Assertions.assertEquals(vr.dif("xi").simple().print(), "1" );
        vr = Expression.convertor("xi*32");
        Assertions.assertEquals(vr.dif("xi").simple().print(), "32");
        vr = Expression.convertor("xi/32");
        Assertions.assertEquals(vr.dif("xi").simple().print(), "0");
